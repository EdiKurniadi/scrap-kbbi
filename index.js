...
kbbiScrapperRouter.get('/entri/:kata', (req, res, next) => {
	...
	request(`https://kbbi.kemdikbud.go.id/entri/${kata}`, (err, respone, html) => {
		if(!err && respone.statusCode === 200) {
			...
			// let data = webHtml('li').text();
			// let datatext = data.map(item => item.text())
			// console.log(data)
			// let data = '';
...
// request(`https://kbbi.kemdikbud.go.id/entri/makan`, (err, respone, html) => {
// 	if(!err && respone.statusCode === 200) {
// 		let webHtml = cheerio.load(html);
// 		// let li = webHtml('li');
// 		// let data =  li.text();
// 		let data = webHtml('li').toArray();
// 		console.log(data)
// 		// if(!data) {data = webHtml('li:first-of-type', 'ul').text()};
// 		// res.status(200).json({
// 		// 	message : 'berhasil mengambil data dari KBBI',
// 		// 	data : data
// 		// })
